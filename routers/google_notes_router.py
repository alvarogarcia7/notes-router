...
Routes messages.10.raw.type.googlenotes to appropriate messages.20.* topics based on type detection.
"""
...
def _detect_message_type(note: dict) -> str:
    ...
    # Default to generic Google Notes
    return "googlenotes"
...
async def route_google_notes(input_msg: dict, client: nats.aio.client.Client) -> None:
    """Route Google Keep notes to appropriate topic based on type detection."""
    ...
    # Get output topic
    output_topic = TYPE_TO_TOPIC.get(msg_type, "messages.20.other.googlenotes")
    ...
    routed_message = {
        ...
        "note": {
            ...
            "labels": original_note.get("labels", []),
...
async def main() -> None:
    """Subscribe to messages.10.raw.type.googlenotes and route appropriately."""
    nc = await _connect_with_retry(NATS_URL)

    print(f"🔄 Google Keep Notes Router started")
    print(f"  Input topic:  {INPUT_TOPIC}")
    print(f"  Output topics:")
    for msg_type, topic in TYPE_TO_TOPIC.items():
        print(f"    - {msg_type}: {topic}")
    print(f"    - default: messages.20.other.googlenotes")
